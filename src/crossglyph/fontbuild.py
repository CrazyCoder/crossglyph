...
# to all four (cpfont/convert.py:1208-1211).
BUNDLED_FALLBACKS = (
...
def build_kwargs(variant: Variant, size: int, out_dir: pathlib.Path) -> dict:
    ...
    four styles itself (cpfont/convert.py:1208-1211), which is why the user, bundled
...
def style_metrics(path: pathlib.Path) -> Metrics:
    ...
    Layout: HEADER_SIZE is 32 (cpfont/convert.py:1198) and STYLE_TOC_FORMAT
    packs each 32-byte style entry (cpfont/convert.py:1242), with glyphCount
