...
get_cache_dir() {
  case "$(uname -s)" in
    Linux)
      echo "${HOME}/.cache/CrossGlyph/tools"
      ;;
    Darwin)
      echo "${HOME}/Library/Caches/CrossGlyph/tools"
      ;;
    *)
...
get_archive_structure() {
  ...
  # __MACOSX is resource-fork metadata emitted by macOS zip tooling — ignore
  # it so archives that happen to ship it (e.g. bun's bun-darwin-*.zip) are
  # still detected as nested rather than falsely treated as flat.
  local top_level_entries
  case "$url" in
    *.tar.gz|*.tgz)
      top_level_entries=$(tar -tzf "$archive" | cut -d'/' -f1 | sort -u | grep -v '^$' | grep -v '^__MACOSX$')
      ;;
    *.zip)
      top_level_entries=$(unzip -Z1 "$archive" | cut -d'/' -f1 | sort -u | grep -v '^$' | grep -v '^__MACOSX$')
      ;;
    *)
  ...
  if [ "$structure" = "flat" ]; then
    ...
    case "$url" in
      ...
      *.zip)
        ...
        # Drop macOS zip metadata so it's not mistaken for the real top-level dir
...
verify_all_platforms() {
  ...
  # Archives are cached by expected sha256, so a stale checksum can never
  # match a new archive — we never need to invalidate entries explicitly.
  # Set TOOL_VERIFY_NO_CACHE=1 to force re-downloads (e.g. to test bandwidth).
  local verify_cache
  verify_cache="$(get_cache_dir)/_verify"
  mkdir -p "$verify_cache"
  ...
  for platform in $platforms; do
    ...
    local cached_archive="$verify_cache/${expected_checksum}"
    local used_cache=0
    if [ -z "${TOOL_VERIFY_NO_CACHE:-}" ] && [ -f "$cached_archive" ]; then
      local cached_sum
      cached_sum=$(compute_checksum "$cached_archive")
      if [ "$cached_sum" = "$expected_checksum" ]; then
        temp_archive="$cached_archive"
        used_cache=1
      else
        rm -f "$cached_archive"
      fi
    ...
    if [ "$actual_checksum" = "$expected_checksum" ]; then
      ...
      # Check that the declared binary exists inside the archive, without
      # extracting — listing is O(directory) and skips writing thousands of
      # files. The on-disk layout after extraction would be:
      #   flat:            <binary>
      #   nested:<dir>/    <dir>/<binary>
      # Zip entries always use forward slashes, so normalize any backslashes
      # in TOOL_BINARY_WINDOWS (e.g. "bin\go.exe" → "bin/go.exe").
      local platform_binary
      platform_binary=$(get_binary_for_platform "$platform")
      local binary_in_archive
      binary_in_archive=$(echo "$platform_binary" | tr '\\' '/')
      if [ "$structure" != "flat" ]; then
        binary_in_archive="${structure#nested:}/$binary_in_archive"
      fi

      local archive_entries
      case "$download_url" in
        *.tar.gz|*.tgz) archive_entries=$(tar -tzf "$temp_archive") ;;
        *.zip)          archive_entries=$(unzip -Z1 "$temp_archive") ;;
      esac

      if echo "$archive_entries" | grep -Fxq "$binary_in_archive"; then
      ...
      # Populate cache after checksum passes, so only known-good archives persist.
      if [ "$used_cache" = "0" ] && [ -z "${TOOL_VERIFY_NO_CACHE:-}" ]; then
        mv -f "$temp_archive" "$cached_archive"
        temp_archive="$cached_archive"
      fi
    else
    ...
    if [ "$used_cache" = "0" ] && [ "$temp_archive" != "$cached_archive" ]; then
      rm -f "$temp_archive"
    fi
    info ""
  done
